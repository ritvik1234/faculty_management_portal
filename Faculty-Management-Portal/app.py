...
app.config['MYSQL_DATABASE_PASSWORD'] = 'harshit123'
...
def insertIntoFaculty(username, name, designation, department, t_leaves=25, a_leaves=25, b_leaves=0, leaves_rqst=False):
    ...
    cursor.execute("INSERT INTO Faculty(username, name, designation, department, total_leaves, avail_leaves, borwd_leaves, leave_rqst) VALUES(%s, %s, %s, %s, %s, %s, %s, %s)",(username, name, designation, department, t_leaves, a_leaves, b_leaves, leaves_rqst))
    connect.commit()
    cursor.close()
...
def insertIntoLeaves(faculty_id, duration, start_date, status='pending', authority_comment='', faculty_comment='',approval_awaited='', sender_dept='', lvl=1, borwd_days=0):
    connect = db.connect()
    cursor = connect.cursor()
    cursor.execute("INSERT INTO Leaves(faculty_id, duration, start_date, status, authority_comment, faculty_comment, approval_awaited, sender_dept, lvl, borwd_days) VALUES(%s, %s, %s, %s, %s, %s,  %s, %s, %s, %s)",(faculty_id, duration, start_date, status, authority_comment, faculty_comment, approval_awaited, sender_dept, lvl, borwd_days))
    connect.commit()
    cursor.close()
...
def saveinfo():
  if request.method == 'POST':
    ...
    profile.insert({'username':session['username'],
                    ...
                    'title':publication_title,
                    'description':publication_desc})
    insertIntoFaculty(session['username'], request.form['name'], request.form['designation'], request.form['department'], t_leaves=25, a_leaves=25, b_leaves=0, leaves_rqst=False)
    if request.form['designation'] == 'Faculty':
      return userprofile()
    else:
      return adminprofile()
...
def updateleave():
  if request.method == 'POST':
    ...
    cursor.execute("UPDATE Leaves SET duration=%s, start_date=%s,status=\'pending\' ,faculty_comment=%s, approval_awaited=%s ,lvl=1, borwd_days=%s WHERE request_id = %s",(form_data['duration'], form_data['start_date'], form_data['comment'],send_to, form_data['borwd_days'], form_data['request_id']))
    connect.commit()
    cursor.close()
...
def leaveaction():
  if request.method == 'POST':
    ...
    if form_data['action'] == 'approved':
      cursor.execute("SELECT * FROM LeaveRoute WHERE lvl = %s AND send_from = %s",(leave_data[0][9]+1,leave_data[0][7]))
      send_to = cursor.fetchall()[0][2]
      ...
      if send_to == leave_data[0][7]: #LEAVE APPROVED
        ...
        cursor.execute("SELECT avail_leaves, borwd_leaves FROM Faculty WHERE faculty_id = %s",(leave_data[0][1]))
        leaves = cursor.fetchall()[0]
        cursor.execute("UPDATE Faculty SET avail_leaves=%s, borwd_leaves=%s, leave_rqst = %s WHERE faculty_id = %s",(leaves[0] - leave_data[0][2],leaves[1] + leave_data[0][10],False, leave_data[0][1]))
        connect.commit()
        cursor.close()
...
def leave():
  if request.method == 'POST':
    ...
    if(int(data['duration']) - int(data['borwd_days'])>res[0][6]):
      ...
      send_to = cursor.fetchall()[0][2]
      insertIntoLeaves(data['faculty_id'], data['duration'], data['start_date'], faculty_comment=data['comment'],authority_comment='', approval_awaited=send_to, sender_dept=res[0][4], lvl=1,borwd_days=data['borwd_days']);
